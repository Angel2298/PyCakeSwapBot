...
if json_config_data["TOKEN_CONTRACT"] == "":
    token = input("Ingrese el contrato del token a comprar\n"
                  "Ingrese aqui: ")
else:
        ...
        if accion == "vender":
            ...
            if live_price_float >= target:
                ...
                if trading:
                    ...
                    get_new_data = True
                    if bool(print("¿Quieres ser notificado a un nuevo precio o quieres salir del programa? "
                          "\nSi: Notificar nuevo precio (cualquier tecla y despues oprime ENTER)"
                          "\nNo: SALIR (Presiona solamente ENTER)")):
                        target = float(input("¿Cual es el nuevo precio al que quieres ser notificado:"
                                             "\nEscribe aqui: "))
                    else:
                        run_program = False
                        sleep(2)
            ...
            # running_cycle = True
            print("volvemos a iniciar")
            print("Aun no ha habido error")
            if live_price_float <= target:
                print("Pasamos a la siguiente fase y no hay error")
                ...
                if trading:
                    ...
                    get_new_data = True
                    if bool(print("¿Quieres ser notificado a un nuevo precio? "
                          "\nSi: Presiona cualquier tecla y despues oprime ENTER"
                          "\nNo: Presiona solamente ENTER")):
                        target = float(input("¿Cual es el nuevo precio al que quieres ser notificado:"
                                             "\nEscribe aqui: "))
                        sleep(2)
...
# Delete all the cookies downloaded from the web driver-manager. Run at the end of the usage.
# driver.delete_all_cookies()
